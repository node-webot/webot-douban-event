...
var error = debug('weixin:model:error');
var verbose = debug('weixin:model:verbose');
var log = debug('weixin:model:log');
  ...
  mongo(function(db) {
    ...
    collection.findOne({ _id: id }, function(err, r) {
      if (err !== null) {
        ...
        return cb.call(cls, err);
      }

      if (r) return cb.call(cls, null,  cls(r));

      log('%s %s not found', kind, id);

      return cb.call(cls, null, null);
    });
  });
};
...
Model.find = function(query, opts, cb) {
  ...
  cls.stream(query, opts, function(stream) {
    if (opts.stream) return cb.call(cls, stream);
...
Model.gets = function(ids, opts, cb) {
  ...
  var self = this;
  return self.find(query, opts, function(err, items) {
    if (err) return cb.call(self, err, items);
...
Model.stream = function(query, opts, cb) {
  ...
  mongo(function(db) {
    cb.call(self, db.collection(self._collection).find(query, opts).stream());
  });
};
  ...
  mongo(function(db, next) {
    ...
    if (_id) {
      ...
      collection.save(obj, {
        ...
        self.emit('saved', res);
        next();
        cb && cb.call(self, err, res);
      });
    } else {
      cb && cb.call(self, 'need id');
    }
  });
};
Model.prototype.update = function(data, cb) {
  ...
  cb.call(self, new Error('missing _id'));
};
